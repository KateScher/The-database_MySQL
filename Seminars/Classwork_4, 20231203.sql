...
INNER JOIN media ON profiles.photo_id = media.id;

SELECT users.id, users.firstname, users.lastname, 
(SELECT hometown FROM profiles WHERE profiles.user_id = users.id), 
(SELECT media.filename FROM media WHERE media.id = (SELECT photo_id FROM profiles WHERE profiles.user_id = users.id))
FROM users;
...
WHERE media.user_id = (
	SELECT users.id FROM users WHERE users.email = 'arlo50@example.org'
);
...
AND media.media_type_id = (
	SELECT media_types.id FROM media_types 
    WHERE media_types.name_type = 'Photo'
);
...
SELECT user_id FROM (
	SELECT initiator_user_id AS user_id, status FROM friend_requests 
	WHERE target_user_id = 1
	UNION
	SELECT target_user_id, status FROM friend_requests 
...
SELECT U.id, U.firstname, U.lastname, P.hometown, M.filename 
FROM users AS U
INNER JOIN profiles AS P ON U.id = P.user_id 
INNER JOIN media AS M ON P.photo_id = M.id;

SELECT U.id, U.firstname, U.lastname, P.hometown, M.filename 
FROM users AS U
LEFT JOIN profiles AS P ON U.id = P.user_id 
LEFT JOIN media AS M ON P.photo_id = M.id;

/*
Задача 5: Список медиафайлов пользователей с количеством лайков (используя JOIN)
*/

SELECT M.filename, U.firstname, U.lastname, L.user_id FROM media AS M
INNER JOIN users AS U ON M.user_id = U.id
INNER JOIN likes AS L ON M.id = L.media_id;

SELECT T.filename, T.firstname, T.lastname, COUNT(T.user_id) FROM 
(
	SELECT M.filename, U.firstname, U.lastname, L.user_id FROM media AS M
	INNER JOIN users AS U ON M.user_id = U.id
	LEFT JOIN likes AS L ON M.id = L.media_id
) AS T
GROUP BY T.filename, T.firstname, T.lastname;


SELECT M.filename, U.firstname, U.lastname, COUNT(L.user_id) FROM media AS M
INNER JOIN users AS U ON M.user_id = U.id
LEFT JOIN likes AS L ON M.id = L.media_id
GROUP BY M.filename, U.firstname, U.lastname;
...
SELECT M.filename, CONCAT(U.firstname, ' ',U.lastname) AS name, MT.name_type FROM media AS M
INNER JOIN users AS U ON M.user_id = U.id
INNER JOIN media_types AS MT ON M.media_type_id = MT.id;

SELECT M.filename, CONCAT(U.firstname, ' ', U.lastname) AS name FROM likes AS L
INNER JOIN users AS U ON L.user_id = U.id
INNER JOIN media AS M ON L.media_id = M.id
WHERE M.id = 1;

SELECT M.filename, CONCAT(U1.firstname, ' ', U1.lastname) AS author, CONCAT(U.firstname, ' ', U.lastname) AS liker FROM likes AS L
INNER JOIN users AS U ON L.user_id = U.id
INNER JOIN media AS M ON L.media_id = M.id
INNER JOIN users AS U1 ON M.user_id = U1.id
WHERE M.id = 1;

SELECT T.filename, T.author, COUNT(T.user_id) FROM 
(
	SELECT M.filename, CONCAT(U.firstname, ' ', U.lastname) AS author, L.user_id FROM media AS M
	INNER JOIN users AS U ON M.user_id = U.id
	LEFT JOIN likes AS L ON M.id = L.media_id AND L.user_id != U.id
) AS T
GROUP BY T.filename, T.author;